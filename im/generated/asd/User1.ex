defmodule User1 do
  ...
  def handle_cast({:chooseAnswer, true}, state) do
    IO.puts("User1: sending #{inspect(1)} to #{inspect(var(state, :server))}")
    GenServer.cast(var(state, :server), {self(), 1})
    {:noreply, state}
  end

  def handle_cast({:chooseAnswer, false}, state) do
    IO.puts("User1: sending #{inspect(2)} to #{inspect(var(state, :server))}")
  ...
  def handle_cast({server, m}, state) when m == 3 do
    ...
    state = updateState(state, %{:m => m, :server => server})
    IO.puts("User1: sending #{inspect(4)} to #{inspect(var(state, :server))}")
